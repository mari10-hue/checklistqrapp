...
function setupEventListeners() {
  btnEnter.addEventListener('click', () => {
    // 1. Vérification de sécurité : L'utilisateur a-t-il scanné un QR Code ?
    const params = new URLSearchParams(window.location.search);
    const qrParam = params.get('qr') || params.get('type');
    const urlStr = window.location.href;
    const hasScanned = qrParam || urlStr.includes('qr=') || urlStr.includes('Daily') || urlStr.includes('Weekly') || urlStr.includes('Monthly');

    if (!hasScanned) {
      document.getElementById('auth-error-msg').textContent = "Accès refusé. Vous devez obligatoirement scanner un QR Code pour commencer.";
    ...
    // Routage spécifique par nom d'utilisateur (Restaurer la structure de connexion avec les emails officiels)
    if (nom.includes('salih') || nom.includes('elyass') || nom.includes('yasse')) {
        ...
        // Utilisateur générique (ex: Marwa)
        currentUser.nom = rawNom;
        currentUser.email = nom.replace(/\s+/g, '.') + '@danone.com';
        
        // Logique de routage QR paramétrée en secours
        const params = new URLSearchParams(window.location.search);
        const qrParam = params.get('qr') || params.get('type');
        const urlStr = window.location.href;
        
        if (qrParam === '1' || qrParam === 'Daily' || urlStr.includes('qr=1') || urlStr.includes('Daily')) {
           openChecklist('factory');
        } else if (qrParam === '2' || qrParam === 'Weekly' || urlStr.includes('qr=2') || urlStr.includes('Weekly')) {
           openChecklist('central');
        } else if (qrParam === '3' || qrParam === 'Monthly' || urlStr.includes('qr=3') || urlStr.includes('Monthly')) {
           openChecklist('director');
        } else {
           navigateTo('menu');
        }
    }
